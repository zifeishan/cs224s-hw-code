...
# tidigits=/farmshare/user_data/zifei/cs224s/TIDIGITS
train_cmd="run.pl"
decode_cmd="run.pl"

echo "DATA DIR:$tidigits"

# The following command prepares the data/{train,dev,test} directories.
traindir=$6
[ -z "$traindir" ] && traindir=4
# traindir=1
...
for x in test train; do
 steps/make_mfcc.sh --cmd "$train_cmd" --nj 20 \
  ...
  case $9 in 
  	0 ) steps/compute_cmvn_stats.sh --fake data/$x exp/make_mfcc/$x $mfccdir || exit 1;;
		1 ) steps/compute_cmvn_stats.sh data/$x exp/make_mfcc/$x $mfccdir || exit 1;;
  esac
...
# steps/train_mono_args.sh  --nj 4 --cmd "$train_cmd" \
#   data/$monodir data/lang exp/mono0a $1 $2 $3 $4

steps/train_mono_args.sh data/$monodir data/lang exp/mono0a $1 $2 $3 $4 $5
...
#    $7 $8 data/train data/lang exp/mono0a_ali exp/tri1
